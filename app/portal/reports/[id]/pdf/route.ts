...
import {getReport,getUserAccess,listAttendanceForReport,listReportElements,listReportPhotos,logActivity} from "../../../../../lib/repositories";
...
export async function GET(request:Request,{params}:{params:Promise<{id:string}>}){
  ...
  const pdf=await generateDailyReportPdf({report,attendance:listAttendanceForReport(report.id),photos:listReportPhotos(report.id),elements:listReportElements(report.id),approvedBy,language:await getPortalLanguage()});
