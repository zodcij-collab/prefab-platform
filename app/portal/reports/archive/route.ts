...
import {getUserAccess,listAttendanceForReport,listProjects,listReportElements,listReportPhotos,listReports,logActivity} from "../../../../lib/repositories";
...
export async function GET(request:Request){
  ...
  const records=reports.map((report)=>({report,attendance:listAttendanceForReport(report.id),photos:listReportPhotos(report.id),elements:listReportElements(report.id),approvedBy:report.approvedById?getUserAccess(report.approvedById)?.name??"":""}));
