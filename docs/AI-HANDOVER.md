...
## CURRENT STATE — Sprint 14 ACCEPTED & MERGED (checkpoint 2026-08-17)

- **Sprint 14 (Drawings, Site Markers & Issue/Drawing Integration V1): DESKTOP MANUAL ACCEPTANCE = COMPLETE (Edvard, 2026-08-17); committed and fast-forward merged to `main` (commit `fbd756d`, "feat(portal): add drawing markers and issue integration") on baseline `8402387`.** Authenticated in-portal pdf.js drawing viewer (`components/portal/DrawingViewer.tsx`; routes `app/portal/projects/[id]/drawings/…`): page nav / zoom / pan / pinch; private PDFs served ONLY via the existing auth'd `…/files/documents/[id]` route; the pdf.js worker + cmaps + standard_fonts are **vendored under `public/pdfjs/` (required for production)**. Markers reuse the Sprint 13 reserved columns (`document_id`, `drawing_page/x/y`) as **normalized 0..1 coords** — the only migration is the additive + idempotent index `sprint14_drawing_markers` (no new tables/columns). Tap-to-create Defect/Task from a drawing (reuses Quick Capture — no phantom issue), set/move/remove a marker on an existing issue, restrained overlay, filters Active/Defects/Tasks/Critical/Resolved/**All** (All also shows Cancelled, subdued). Issue Detail deep-link + **contextual Back** (explicit project-scoped return contract, refresh/bookmark-safe). **Issue PDF drawing-location**: a human-readable relative position (never raw coordinates) + a **client-captured drawing crop** stored as an `issue_media` row with role `drawing-location` and embedded via pdfkit (textual fallback; a missing/broken snapshot can never fail PDF generation). Access & Roles: full **LV / EN / RU** localization audit (new `fixpack14Text` in `data/portal-i18n.ts`), **user deactivate/reactivate** lifecycle (soft + audited + session-invalidating; `users.active`, `repositories.setUserActiveState` — no hard delete, history preserved), and role-vs-project-access UX. RBAC fix: **"Default for role" project access** now writes an explicit **empty-capability membership row** (`{}` → project visible with role-derived capabilities) instead of deleting the grant; `resolveProjectCapabilities` unchanged. Two-layer RBAC (role/capabilities × explicit project access) preserved; no auto-grant. `npm test` = **228 passing**; lint / typecheck / build clean. Latest verified pilot backup: `backups/pilot-sprint14-final-2026-08-17T08-45-43Z.db` (`integrity_check` ok, FK clean). Pilot counts: 5 users · 671 elements · 7 issues · 4 markers · 9 media (2 drawing-location) · 9 documents · 1 project-permission. **Deferred (intentional):** server-side rasterized PDF page snapshot (blocked by `@napi-rs/canvas` Path2D glyph fill under pdfjs 4.10 — delivered instead as the client-canvas crop above); real-device / mobile acceptance (pending a staging environment); scope-excluded DWG/DXF/BIM/OCR/AI/annotation tools/WebSockets/offline/email/RFI.

<!-- Sprint 13 history retained below. -->
