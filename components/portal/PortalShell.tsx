...
const nav=[["/portal","Dashboard","DB"],["/portal/projects","Projects","PR"],["/portal/employees","Employees","HR"],["/portal/documents","Documents","DO"],["/portal/reports","Reports","RP"],["/portal/access","Access & roles","AC"]] as const;

export async function PortalShell({children,active}:{children:ReactNode;active:string}){const language=await getPortalLanguage();const t=(value:string)=>portalText(language,value);return <main className="os-shell">
  <aside className="os-sidebar"><Link className="os-brand" href="/">PREFAB<span>.LV</span></Link><div className="os-product">{t("Corporate Platform")}</div>
    <nav className="os-nav" aria-label={t("Portal navigation")}>{nav.map(([href,label,code])=>{const localizedLabel=t(label);const isActive=active===href;return <Link aria-current={isActive?"page":undefined} aria-label={localizedLabel} className={isActive?"active":""} href={href} key={href} title={localizedLabel}><span aria-hidden="true" className="os-nav-code">{code}</span><span className="os-nav-label">{localizedLabel}</span></Link>})}</nav>
