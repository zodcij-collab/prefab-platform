...
// Sensible, content-based column widths so the workbook is readable without manual
// resizing: at least the header width, capped so no column becomes unwieldy.
export function computeColumnWidths(rows:(string|number)[][],min=10,max=48):number[]{
  const count=rows.reduce((widest,row)=>Math.max(widest,row.length),0),widths:number[]=[];
  for(let column=0;column<count;column++){let longest=0;for(const row of rows){const length=String(row[column]??"").length;if(length>longest)longest=length;}widths.push(Math.min(max,Math.max(min,longest+2)));}
  return widths;
}
function sheetXml(rows:(string|number)[][]){
  ...
  const body=rows.map((row,rowIndex)=>`<row r="${rowIndex+1}">${row.map((value,columnIndex)=>{const ref=`${columnName(columnIndex)}${rowIndex+1}`;return typeof value==="number"?`<c r="${ref}"${rowIndex===0?' s="1"':""}><v>${value}</v></c>`:`<c r="${ref}" t="inlineStr"${rowIndex===0?' s="1"':' s="2"'}><is><t xml:space="preserve">${xml(value)}</t></is></c>`;}).join("")}</row>`).join("");
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="15"/>${cols}<sheetData>${body}</sheetData><autoFilter ref="A1:${columnName(Math.max(0,(rows[0]?.length??1)-1))}${Math.max(1,rows.length)}"/></worksheet>`;
}
...
export function timesheetWorkbook(entries:TimeEntry[],labels:ExportLabels=englishExportLabels):Uint8Array{
  ...
  const files:Record<string,Uint8Array>={
    ...
    "xl/styles.xml":strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Aptos"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Aptos"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF1E293B"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf></cellXfs></styleSheet>`),
